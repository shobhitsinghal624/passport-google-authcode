...
GoogleAuthCodeStrategy.prototype._exchangeAuthCode = function(authCode, done) {
  ...
  this._oauth2._request('POST', 'https://accounts.google.com/o/oauth2/token', post_headers, post_data, null, function (err, body, res) {
    if (err) { return done(new InternalOAuthError('failed to exchange authorization code for access token', err)); }
    var results;
    try {
